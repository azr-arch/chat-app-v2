...
export const AddUserModal = ({ friends }: AddUserModalProps) => {
    ...
    const { isOpen, onClose, type: modalType } = useAddFriendModal();
                                ...
                                <ul className="w-full flex flex-col items-start">
                                    {searchResult ? (
                                        <li className="w-full">
                                            <div className="w-full flex items-center gap-x-4">
                                                <Image
                                                    ...
                                                    height={50}
                                                    width={50}
                                                    className="rounded-full shadow-sm w-auto h-auto"
                                                />
                                                ...
                                                <Button
                                                    ...
                                                    variant={"secondary"}
                                                    onClick={() =>
                                                        onMessage(searchResult.email as string)
                                                    }
                                                    disabled={loading}
                                                >
                                                    Message
                                        ...
                                        <span className="absolute top-1/2 text-neutral-600 -translate-y-1/2 left-1/2 -translate-x-1/2 w-full text-center">
                                            No friends found. Try adjusting your search or invite
                                            friends to join!
                                        </span>
                                    )}
                                </ul>
